...
const all_instractors = ({ allInstractors }) => {
  ...
  const handleDelete = (id) => {
    console.log(id);
    const url = `https://digital-learning-ruddy.vercel.app/api/users/${id}`;
    fetch(url, {
      method: "DELETE",
      headers: {
      ...
      <div className="pt-5 lg:pt-10 px-5 lg:px-40 mt-10 bg-neutral text-black">
        ...
        <div className="overflow-x-auto w-full  pb-10">
          <table className="table w-full">
            ...
            <tbody>
              {allInstractors.map((instrator) => (
                <tr key={instrator._id}>
                  <td>
...
export async function getServerSideProps() {
  ...
  const res = await fetch(
    `https://digital-learning-ruddy.vercel.app/api/users/all_instractors`
  );
